...
class KnowledgeBaseCLI {
  ...
  async createLink(name, sourcePath, linkName = null, targetDir = null) {
    if (!linkName) {
      linkName = name;
    }
  ...
  async setupKnowledgeBase(name, config, targetDir = null) {
    ...
    if (localPaths.length > 0) {
      ...
      if (existingPath) {
        return await this.createLink(name, existingPath, null, targetDir);
      }
    }
    ...
    // 3. 创建软链接
    return await this.createLink(name, clonedPath, null, targetDir);
  }
