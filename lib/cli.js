...
class KnowledgeBaseCLI {
  ...
  async addToGitignore(linkName, targetDir) {
    const gitignorePath = path.join(targetDir, '.gitignore');

    try {
      let gitignoreContent = '';

      // 如果.gitignore文件存在，读取内容
      if (fs.existsSync(gitignorePath)) {
        gitignoreContent = fs.readFileSync(gitignorePath, 'utf8');
      }

      // 检查是否已经存在忽略规则
      const ignoreRule = `# KG-CLI 知识库软链接\n${linkName}/\n`;

      if (gitignoreContent.includes(`${linkName}/`)) {
        console.log(
          chalk.yellow(`⚠️  .gitignore 中已存在忽略规则: ${linkName}/`),
        );
        return true;
      }
      ...
      console.log(chalk.green(`✅ 已添加到 .gitignore: ${linkName}/`));
      return true;
    } catch (error) {
      console.log(chalk.red(`❌ 更新 .gitignore 失败: ${error.message}`));
      return false;
    }
  }
  ...
  async createLink(
    ...
    const success = await this.createSymlink(sourcePath, linkPath);

    // 如果软链接创建成功且不跳过gitignore，添加到.gitignore
    if (success && !skipGitignore) {
      await this.addToGitignore(linkName, targetDir);
    }
